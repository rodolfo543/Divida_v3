# -*- coding: utf-8 -*-
"""
AXS Energia Unidade 11 - 1a emissao de debentures em serie unica
Calculo PMT padrao ANBIMA com projecao Focus/BCB.

Escopo desta versao
- Calcula a Serie Unica da 1a emissao da AXS 11.
- Mantem o mesmo padrao dos motores IPCA recentes do projeto.
- Exporta CSV e XLSX com o fluxo completo.

Premissas principais extraidas da escritura e do 2o aditivo
- Data de emissao: 15/09/2025.
...
- Juros: base 252 dias uteis, com 3 incorporacoes iniciais em:
  15/03/2026, 15/09/2026 e 15/03/2027.
- Primeiro pagamento de juros e primeira amortizacao: 15/09/2027.
- Amortizacao programada em 29 parcelas semestrais ate 15/09/2041.
- Taxa anual: 10,7385% a.a., conforme 2o aditivo.

Premissas operacionais adicionais
- Ticker adotado: AXSI11.
- ISIN adotado: BRAXSIDBS004.
- Data de inicio da rentabilidade: a escritura define a data como a primeira
  integralizacao. Esta versao adota 25/09/2025, em linha com o historico legado
  ja utilizado no projeto para a AXS 11. Se houver documento com outra data
  efetiva de primeira integralizacao, ajuste o parametro abaixo e rode novamente.

Arquivos gerados
- controle_divida_axs11_v1.csv
- controle_divida_axs11_v1.xlsx
"""
...
DATA_EMISSAO = date(2025, 9, 15)
DATA_INICIO_RENTABILIDADE = date(2025, 9, 25)
...
IPCA_FALLBACK_VARIACAO_MENSAL = {
    "2025-07": Decimal("0.0026"),
    "2025-08": Decimal("-0.0011"),
    "2025-09": Decimal("0.0048"),
    "2025-10": Decimal("0.0009"),
    "2025-11": Decimal("0.0018"),
    "2025-12": Decimal("0.0033"),
    "2026-01": Decimal("0.0033"),
    "2026-02": Decimal("0.0070"),
    "2026-03": Decimal("0.0088"),
}


@dataclass(frozen=True)
class DebentureConfig:
    nome: str
    ticker: str
    isin: str
    quantidade: Decimal
...
DEBENTURE = DebentureConfig(
    nome="AXS 11 - Serie Unica",
    ticker="AXSI11",
    isin="BRAXSIDBS004",
    quantidade=Decimal("170000"),
    taxa_aa=Decimal("0.107385"),
    cronograma=[
        (datetime.strptime(data_txt, "%Y-%m-%d").date(), Decimal(valor) / Decimal("100"))
        for data_txt, valor in CRONOGRAMA_RAW
    ],
    observacao_taxa="Taxa anual adotada conforme 2o aditivo da escritura e informacao publica da emissao.",
)
...
def feriados_nacionais(start_year: int = 2025, end_year: int = 2041) -> set[date]:
    fs: set[date] = set()
...
def periodo_final_sidra() -> str:
    max_aniv = max(aniversario(d) for d, _ in DEBENTURE.cronograma)
...
def obter_ipca_numero_indice_sidra() -> Tuple[Dict[str, Decimal], str]:
    ...
    url = f"https://apisidra.ibge.gov.br/values/t/1737/n1/all/v/2266/p/202507-{fim}?formato=json"
    try:
...
def preencher_indices_futuros(indices: Dict[str, Decimal]) -> Tuple[Dict[str, Decimal], Dict[str, str]]:
    ...
    fim = mes_str(add_months(max(aniversario(d) for d, _ in DEBENTURE.cronograma), -1))
...
def calcular_fluxo(
    ...
    for i, data_incorp in enumerate(DATAS_INCORPORACAO_JUROS, start=1):
        ...
        fj_incorp = fator_juros_252(du_incorp, DEBENTURE.taxa_aa)
        ...
        linhas.append({
            ...
            "Saldo_Devedor_R$": round_dec(saldo_pu * DEBENTURE.quantidade, 2),
        })
        data_ref_juros = data_incorp

    for data_pagto, perc_amort in DEBENTURE.cronograma:
        ...
        fj = fator_juros_252(du, DEBENTURE.taxa_aa)
        ...
        juros_rs = round_dec(pu_juros * DEBENTURE.quantidade, 2)
        amort_rs = round_dec(pu_amort * DEBENTURE.quantidade, 2)
        pmt_rs = round_dec(juros_rs + amort_rs, 2)
        saldo_rs = round_dec(pu_vna_fim * DEBENTURE.quantidade, 2)
...
def salvar_xlsx(linhas: List[Dict[str, object]], caminho: Path, titulo: str, fonte_ipca: str, observacao: str) -> None:
    ...
    tab = Table(displayName="TabelaControleAXS11", ref=ref)
...
def main() -> None:
    ...
    linhas = calcular_fluxo(indices, fonte_mes)

    arquivo_csv = BASE_DIR / "controle_divida_axs11_v1.csv"
    arquivo_xlsx = BASE_DIR / "controle_divida_axs11_v1.xlsx"

    salvar_csv(linhas, arquivo_csv)
    try:
        salvar_xlsx(linhas, arquivo_xlsx, DEBENTURE.nome, fonte, DEBENTURE.observacao_taxa)
        print(f"Arquivo XLSX gerado: {arquivo_xlsx.name}")
    except Exception as exc:
    ...
    print("Premissa taxa:", DEBENTURE.observacao_taxa)
    print("Premissa data inicio rentabilidade:", DATA_INICIO_RENTABILIDADE.strftime("%d/%m/%Y"))
    imprimir_linha("15/09/2027", linhas)
    imprimir_linha("15/09/2041", linhas)
